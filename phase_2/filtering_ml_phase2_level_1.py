...
class preprocess():
    def dataNormalize(df):
        ...
        df_normalized['S1_ind']=df['S1_ind']
        df_normalized['Richness_ind']=df['Richness_ind']
        df_normalized['S1_Richness_efficiency']=df['S1_Richness_efficiency'] 
        df_normalized['S1_Richness_balance']=df['S1_Richness_balance']
        
        
        df_normalized['CodeA']=df['CodeA']
        df_normalized['CodeB']=df['CodeB']
        df_normalized['CodeC']=df['CodeC']
    ...
    def dataPreprocess(df):
        #df=df[(df['S1_COUNT']>0)&(df['Richness_COUNT']>0)]
        gc.collect()
...
class evaluation():
    ...
    def Visualize(x,df_similarity,y_predict):
        ...
        df_3 = df_similarity[(df_similarity['CodeC']==42) & (df_similarity['CodeB']==210) & (df_similarity['CodeA']==104)]
        df_4 = df_similarity[(df_similarity['CodeC']==42) & (df_similarity['CodeB']==687) & (df_similarity['CodeA']==327)]
        df_5 = df_similarity[(df_similarity['CodeC']==650) & (df_similarity['CodeB']==22) & (df_similarity['CodeA']==295)]
        df_6 = df_similarity[(df_similarity['CodeC']==1949) & (df_similarity['CodeB']==628) & (df_similarity['CodeA']==228)]
        df_7 = df_similarity[(df_similarity['CodeC']==1493) & (df_similarity['CodeB']==703) & (df_similarity['CodeA']==315)]
        df_8 = df_similarity[(df_similarity['CodeC']==1754) & (df_similarity['CodeB']==628) & (df_similarity['CodeA']==228)]
        df_9 = df_similarity[(df_similarity['CodeC']==42) & (df_similarity['CodeB']==699) & (df_similarity['CodeA']==327)]
        df_10 = df_similarity[(df_similarity['CodeC']==1451) & (df_similarity['CodeB']==187) & (df_similarity['CodeA']==228)]
        df_10 = df_similarity[(df_similarity['CodeC']==1451) & (df_similarity['CodeB']==187) & (df_similarity['CodeA']==228)]
        ...
        for i in range(len(x_sample_1355)):
            plt.text(x_sample_1355[i],y_sample_1355[i]+0.2,text_sample[i])
        plt.legend(loc='best')
        fig_2=plt.gcf()
        ...
        for i in range(len(x_sample_1361)):
            plt.text(x_sample_1361[i],y_sample_1361[i]+0.2,text_sample[i])
        plt.legend(loc='best')
        fig_3=plt.gcf()
        ...
        plt.scatter(x_erh_1355[:,0],x_erh_1355[:,1], alpha=0.3)
        points = [(x,x) for x in range(0,12000)]
        x_line,y_line = zip(*points)
        plt.plot(x_line,y_line)
        plt.xlim(0,12000)
        plt.ylim(0,12000)
    ...
    def score(df_similarity):
        ...
        df_similarity_1['Score']=(df_similarity_1['Similarity_RANK']+(df_similarity_1['Similarity_point_mixed_RANK_pY1355']+df_similarity_1['Similarity_point_mixed_RANK_pY1361'])/2).astype(float)
        ...
        df_similarity_0['Score']=(df_similarity_0['Similarity_RANK']+(df_similarity_0['Similarity_point_mixed_RANK_pY1355']+df_similarity_0['Similarity_point_mixed_RANK_pY1361'])/2).astype(float)
        #df_similarity_0['Score']=(df_similarity_0['Similarity_RANK']+(df_similarity_0['Similarity_antiinsr_RANK_pY1355']+df_similarity_0['Similarity_antiinsr_RANK_pY1361'])/2).astype(float)
        ...
        df_sample = pd.concat((df_0,df_1,df_2,df_3,df_4,df_6),axis=0)
        df_sample.to_csv('sample_phase2_20241017.csv')
        #print(dist)
        result_name = 'output_'+model_name+'_phase2_level1.csv'
        df_score.to_csv(output_dir+result_name)
        return df_score
if __name__=='__main__':
    
    data_dir = 'C:/Users/sharo/Documents/chemistry/output/phase_2/level_0/output_OneClassSVM_phase2_level0.csv'
    ...
    graph_dir='C:/Users/sharo/Documents/chemistry/graph/phase_2/level_1/'
    output_dir='C:/Users/sharo/Documents/chemistry/output/phase_2/level_1/'
    temp_dir = 'C:/Users/sharo/Documents/chemistry/parameters/phase_2/level_1/'
    ...
    y_predict,model_name,x,df_result =similarityAnalysis.classification(df_erh_insr)
