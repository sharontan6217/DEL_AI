...
def Visualize(x,df_similarity,y_predict,df_score,samples,opt):
	...
	for i in range(len(x_sample_1355)):
		if df_sample['CodeA'][i]==169 and df_sample['CodeB'][i]==247 and df_sample['CodeC'][i]==192:
	...
	for i in range(len(x_sample_1361)):
		...
		if df_sample['CodeA'][i]==134 and df_sample['CodeB'][i]==207 and df_sample['CodeC'][i]==192:
			...
			plt.text(x_sample_1361[i],y_sample_1361[i]-0.1,text_sample[i],fontsize='small',fontweight='bold')
	...
	plt.scatter(x_erh_1355[:,0],x_erh_1355[:,1], alpha=0.3)
	#points = [(x,x) for x in range(0,5000)]
	points = [(x,x) for x in range(0,15000)]
	x_line,y_line = zip(*points)
	plt.plot(x_line,y_line)
	plt.xlim(0,15000)
	...
	for i in range(len(x_sample)):
		if df_sample['CodeA'][i]==104 and df_sample['CodeB'][i]==210 and df_sample['CodeC'][i]==1457:
			...
			print(x_sample[i],y_sample_1361[i],text_sample[i])
		else:
			print(x_sample[i],y_sample_1361[i],text_sample[i])
			plt.text(x_sample[i],y_sample_1361[i],text_sample[i],fontsize='small')
	...
	for i in range(len(x_sample)):
		if df_sample['CodeA'][i]==235 and df_sample['CodeB'][i]==245 and df_sample['CodeC'][i]==192:
			ax.text(x_sample[i],y_sample[i]-0.2,z_sample[i]-0.2,text_sample[i],fontsize='small')
			print(x_sample[i],y_sample_1361[i],text_sample[i])
		else:
			print(x_sample[i],y_sample_1361[i],text_sample[i])
			ax.text(x_sample[i],y_sample[i]+0.2,z_sample[i],text_sample[i],fontsize='small')
...
def Visualize_tpor(x,df_similarity,y_predict,df_score,samples,opt):
	...
	ax.set_zlabel('PCA 3')
	for i in range(len(x_sample)):
		if df_sample['CodeA'][i]==124 and df_sample['CodeB'][i]==159 and df_sample['CodeC'][i]==422:
			ax.text(x_sample[i],y_sample[i]+0.1,z_sample[i]+0.1,text_sample[i],fontsize='small')
		else:
			ax.text(x_sample[i],y_sample[i],z_sample[i],text_sample[i],fontsize='small')
	plt.legend(loc='best')
	fig_12=plt.gcf()
